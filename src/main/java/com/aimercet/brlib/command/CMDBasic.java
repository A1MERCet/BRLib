...
public abstract class CMDBasic implements CommandExecutor
{
    public enum ArgType
    {
        DEPEND("前置"){
            @Override public Object format(String v) {return null;}},
        INTEGER("整型"){
            @Override public Object format(String v) {try {return Integer.parseInt(v);}catch (Exception e){return null;}}},
        STRING("字符"){
            @Override public Object format(String v) {try {return v;}catch (Exception e){return null;}}},
        FLOAT("浮点"){
            @Override public Object format(String v) {try {return Float.parseFloat(v);}catch (Exception e){return null;}}},
        BOOLEAN("布尔"){
            @Override public Object format(String v) {try {return Boolean.valueOf(v);}catch (Exception e){return null;}}},
        ;
        public final String name;
        ArgType(String name) {this.name = name;}
        public Object format(String v){return null;}
    }
    ...
    public CMDBasic(String name, Class<?> clazz) {
        this.name = name;
        this.clazz=clazz;
    ...
    {
        if (senderCheck(sender) && command.getName().equalsIgnoreCase(name))
        {
    ...
    public Object invoke(CommandSender sender , Method method , String[] args)
    {
        try {
            ...
            for (int i = 0;i<anTypes.length;i++) {
                ...
                Object formart = type.format(args[i]);
